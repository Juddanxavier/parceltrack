...
export function isAuthenticated(req: Request, res: Response, next: NextFunction) {
  getSessionWithRole(req.headers)
    .then((session) => {
      if (session && session.user) {
        req.user = session.user; // You might need to create an interface to include `user`
        next();
      } else {
...
export async function getSessionWithRole(headers: any) {
